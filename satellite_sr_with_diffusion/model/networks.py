...
def weights_init_kaiming(m, scale=1):
    """Initializes weights with Kaiming (He) normal distribution."""
    classname = m.__class__.__name__
...
def weights_init_orthogonal(m):
    """Initializes weights with an orthogonal distribution."""
    classname = m.__class__.__name__
    if classname.find("Conv") != -1 or classname.find("Linear") != -1:
...
def init_weights(net, init_type="kaiming", scale=1, std=0.02):
    ...
    Args:
        ...
        scale (float): Scaling factor for Kaiming initialization.
...
def define_G(opt):
    """
    Defines the generator network based on the provided options.
