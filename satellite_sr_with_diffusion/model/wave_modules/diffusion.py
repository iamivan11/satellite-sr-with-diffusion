...
class GaussianDiffusion(nn.Module):
    ...
    def apply_idwt(self, features, targetW, targetH):
        ...
        return F.interpolate(sr_images, size=(targetW, targetH), mode="bicubic")
    ...
    def p_sample_loop(self, x_in, continous=False, sample_inter=None):
        ...
        if continous:
            ...
            result = torch.cat(
                [result, self.apply_idwt((ret_img[0] + x_sr[0]).unsqueeze(0), w, h)], 0
            )
            for i in range(1, len(ret_img)):
                result = torch.cat(
                    [result, self.apply_idwt((ret_img[i] + x_sr[0]).unsqueeze(0), w, h)], 0
                )
            return result
    ...
    def sample(self, batch_size=1, continous=False, sample_inter=None):
        ...
        return self.p_sample_loop(
            (batch_size, channels, image_size, image_size), continous, sample_inter
        )
    ...
    def p_losses(self, x_in, noise=None, return_reconstruction=False):
        x_start = self.apply_dwt(x_in["HR"])
        [b, _c, _h, _w] = x_start.shape
        ...
        if not return_reconstruction:
            return loss
        # Reconstruct the image from noise for LPIPS calculation
        pred_x_start_dwt_residual = (
            x_noisy
            - (1 - continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1) ** 2).sqrt() * predicted_noise
        ) / continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1)
        pred_x_start_dwt = pred_x_start_dwt_residual + x_sr

        # Apply inverse DWT to get image in pixel space
        _, _, h_img, w_img = x_in["HR"].shape
        pred_hr_img = self.apply_idwt(pred_x_start_dwt, h_img, w_img)

        return {"loss": loss, "x_recon": pred_hr_img}

    def forward(self, x, return_reconstruction=False, *args, **kwargs):
        return self.p_losses(x, *args, return_reconstruction=return_reconstruction, **kwargs)
