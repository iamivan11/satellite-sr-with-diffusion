...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_in, noise=None, return_reconstruction=False):
        ...
        if not return_reconstruction:
            return loss
        x_recon_img = self.predict_start_from_noise(
            x_noisy, t=torch.zeros(b, device=x_start.device, dtype=torch.long), noise=x_recon
        )
        return {"loss": loss, "x_recon": x_recon_img}
