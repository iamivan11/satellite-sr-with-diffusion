...
def safe_rename_directory(directory: Path, description: str):
    ...
    try:
        ...
        image_files = sorted(
            [f for f in os.listdir(directory) if Path(f).suffix.lower() in DEFAULT_EXTS]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Safely rename dataset images to sequential numbers."
    )
    parser.add_argument(
        "--base-dir", type=Path, default=DEFAULT_BASE_DIR, help="Path to the dataset base directory"
    )
    parser.add_argument(
        "--splits", type=str, nargs="*", default=DEFAULT_SPLITS, help="Splits to process"
    )
    parser.add_argument(
        "--folders",
        type=str,
        ...
        help="Subfolders to process (e.g., hr lr)",
    )
    ...
    for split in args.splits:
        ...
        for folder in args.folders:
            ...
            safe_rename_directory(
                directory_path, description=f"{split.capitalize()} {folder.upper()}"
            )
